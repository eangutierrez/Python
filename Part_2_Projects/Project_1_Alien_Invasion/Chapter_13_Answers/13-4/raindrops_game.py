...
class RaindropsGame:
    ...
    def _update_raindrops(self):
        """
        Update drop positions and look for drops that have dissapeared
        """
        self.raindrops.update()

        # Assume that there's no need to create new drops.
        make_new_drops = False
        for drop in self.raindrops.copy():
            if drop.check_disappeared():
                # Remove thid drop, and make new drops
                self.raindrops.remove(drop)
                make_new_drops = True
        
        # Make new drop rows if needed
        if make_new_drops:
            self._create_row(0)
