...
class Target:
    ...
    def __init__(self, ss_game):
        ...
        # Create a target rect at (0, 0) and then set its coords
    ...
    def update(self):
        """Move the target up and down in a pattern."""
        # Update the decimal position of the target
        self.y += self.direction * self.settings.target_speed

        if self.rect.top < 0:
            # target moved past the top of the screen
            # put it on top of screen, change direction
            self.rect.top = 0
            self.direction = 1
    ...
    def center_target(self):
        ...
        # Store target position as a decimal value
